...
struct inode {
	...
	unsigned long		i_ino;		/* 唯一的编号标识 */
	atomic_t		i_count;		/* 指定访问该inode结构的进程数目 */
	unsigned int			i_nlink;		/* 记录使用该inode的硬链接的数目 */
	/* i_uid and i_gid (the UID and GID associated with the file). */
	uid_t			i_uid;
	gid_t			i_gid;
	dev_t			i_rdev;		/* 当inode表示设备文件时，i_rdev表示与哪个设备进行通信。*/
...
#endif
	/*
	 i_atime, i_mtime, and t_ctime store the time of the last access, 
     the time of the last modification, and the time of the last inode change. 
     */
	...
	/*
	 file_operations 用于操作文件中包含的数据。而inode_operations
     负责管理结构性的操作（例如删除一个文件）和文件相关的元数据（例如：属性）。
     */
	...
	union {
		struct pipe_inode_info	*i_pipe;	/* 包含了用于实现管道的inode的相关信息 */
