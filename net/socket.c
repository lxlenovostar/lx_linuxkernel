			 ...
			 struct socket **res, int kern)
{
	 ...
	 */
	/*
     sock_alloc, called from sock_create, returns an allocated socket 
	 structure. The socket structure is actually part of an inode structure, 
     created when sock_alloc calls new_inode. It is necessary to
     have an inode for Linux IO system calls to work with sockets.
     */
	sock = sock_alloc();
	if (!sock) {
		if (net_ratelimit())
 ...
 */
/*
 文件功能中的读写操作可以通过虚拟文件系统相关系统调用进入内核，然后重定向到 socket_file_ops 
 结构的函数指针，除此之外，还需要对套接字执行其他任务，这些不能融入到文件方案中，比如创建套接字、
 bind、listen等，因此，Linux 提供了 socketcall 系统调用，它实现在 sys_socketcall 中。
 */
