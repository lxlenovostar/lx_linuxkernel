...
set for the socket.*/
    ...
    */
	...
	/* Ok commence sending. */
	iovlen = msg->msg_iovlen;   /* get the number of the user buffers */
	iov = msg->msg_iov;         /* the pointer to the user buffer */
	copied = 0;
    ...
    */
	while (--iovlen >= 0) {
		...
		while (seglen > 0) {
			...
			skb = tcp_write_queue_tail(sk); /* the last segment is checked whether some room don't use. */

            /* sk_send_head points to the socket buffers queued for transmitting.*/
            /* If sk_send_head is null, there are not yet any segments queued for sending. */
            /* check if length of the segment is less than the current mss */
			if (!tcp_send_head(sk) ||
...
new_segment:
                ...
                /* select_size to round off the requested size to 
                    the nearest TCP segment size or page size
                    determined from the MSS.
                */
                /*
                    If our hardware is aware of the scatter – gather technique, we
allocate a buffer that fits into a single page. Otherwise, we get a buffer of length
1 mss (buffer that can hold 1 mss of TCP payload). 
            ...
            */
			if (skb_tailroom(skb) > 0) {
                ...
                */
				...
				struct page *page = TCP_PAGE(sk);   /*  page is declared to point to a memory-mapped page */
				int off = TCP_OFF(sk);  /*off is set to the offset to the start of the data to be copied from the msghdr structure.*/
                
                /* skb_can_coalesce checks to see if there is space in the last page in the socket buffer */
				...
				if (merge) {
                    ...
                    /* 
                        fill_page_desc is called, which updates the frag array in
                        the skb with information about the new page. 
                    */
            ...
            /* 
                A zero value in the variable copied indicates that this is 
                the first time  through the while loop for the initial segment 
                so the PSH flag in the TCP header is set to zero.
            */
			if (!copied)
				TCP_SKB_CB(skb)->flags &= ~TCPCB_FLAG_PSH;

            /*
                The write_seq field in the TCP options structure is updated with 
                the amount of data processed in this trip through the while loop. 
                The end_seq field in the TCP control block is also updated with
            ...
            */
            ...
            /*
                From, which points to the data source in the msg argument, and copied, 
                which holds the total number of bytes processed so far, are updated for
                this iteration. Iovlen is the number of iovs in the msghdr structure, 
                and seglen was initialized to this value at the start of the function.
            */
			from += copy;
			copied += copy;
            ...
            /*
                At this point, we decide if small segments should be pushed out or held. 
                We call forced_push to  check if we must send now no matter what the segment 
                size is. If so, the PSH flag is set and the segments are transmitted.
            */
			if (forced_push(tp)) {
				tcp_mark_push(tp, skb);
				__tcp_push_pending_frames(sk, mss_now, TCP_NAGLE_PUSH);
            ...
            /*
                This is where tcp_sendmsg ends up if there is insufficient number of buffers 
                or pages. It waits until there are a sufficient number of buffers available.
            */
wait_for_sndbuf:
			set_bit(SOCK_NOSPACE, &sk->sk_socket->flags);
wait_for_memory:
    ...
    /*
        This is the label where the code jumped if it is time to push 
        the remaining data and exit the function. The value of copied 
        is returned, indicating to the application program how much data
        was transmitted.
    */
out:
	if (copied)
		...
		size_t len, int nonblock, int flags, int *addr_len)
{
	...
	seq = &tp->copied_seq; //copied_seq is the position of the start of received data that has not yet been read by the user process.
	if (flags & MSG_PEEK) {
		peek_seq = tp->copied_seq;
		seq = &peek_seq;
	}

	target = sock_rcvlowat(sk, flags & MSG_WAITALL, len); //sk_rcvlowat : set the low-water mark for the socket. 
	...
	do {
		u32 offset; //the number of bytes to copy from the skb.
        ...
        //The skb_peek() function is used to check to see if a queue is empty. If it is not empty a pointer to the first element is returned.
		skb = skb_peek(&sk->sk_receive_queue);  //sk_receive_queue is the receive queue.
		do {
			if (!skb)
				break;
		...
		if (!(flags & MSG_TRUNC)) {
...
#endif
			{
				err = skb_copy_datagram_iovec(skb, offset,
						msg->msg_iov, used);    //This is where we copy the data to user space.
				if (err) {
					/* Exception. Bailout! */
					if (!copied)
						copied = -EFAULT;
...
skip_copy:
	...
	found_fin_ok:
		/* Process the FIN. */
		++*seq;
		if (!(flags & MSG_PEEK)) {  //MSG_PEEK: Returns data from the head of the receive queue without removing it.
			sk_eat_skb(sk, skb, copied_early);
			copied_early = 0;
		}
		break;
