...
void tcp_parse_options(struct sk_buff *skb, struct tcp_options_received *opt_rx, int estab)
{
	...
	int length=(th->doff*4)-sizeof(struct tcphdr);  //TCP选项的长度

	ptr = (unsigned char *)(th + 1);
	opt_rx->saw_tstamp = 0;

	while (length > 0) {
		int opcode=*ptr++;  //按字节在增长
		int opsize;
		...
		switch (opcode) {
			...
			default:
				...
				case TCPOPT_SACK:
                    /*
                     * 第一个判断是否剩下的长度大于选项类型、选项长度和单个SACK块情况下的长度。
                     * 第二个判断SACK的块格式是否符合要求。
                     */
					if ((opsize >= (TCPOLEN_SACK_BASE + TCPOLEN_SACK_PERBLOCK)) &&
					   !((opsize - TCPOLEN_SACK_BASE) % TCPOLEN_SACK_PERBLOCK) &&
					   opt_rx->sack_ok) {
						TCP_SKB_CB(skb)->sacked = (ptr - 2) - (unsigned char *)th; //保存偏移地址
					}
					break;
